...
export default function Form({ record, data: { id, formFields, confirmation } }: ButtonBlockProps) {
	const [formValues, setFormValues] = useState({ fromName: '', fromEmail: '', confirmation });
	...
	const handleSubmit: React.FormEventHandler<HTMLFormElement> = (e) => {
		...
		const form = { recordId: record.id, fromEmail, fromName, fields, confirmation };
		...
		fetch('/api/contact-form', {
			...
			.then(async (res) => {
				...
				if (success === true) setSuccess(true);
				else if (error) setError(new Error(error));
	...
	return (
		<section className={s.form}>
			...
			{success ? (
				<p ref={confirmationRef} className={s.confirmation}>
					{confirmation}
				</p>
			) : (
				<form onSubmit={handleSubmit} className={cn(loading && s.loading)}>
					<input id='confirmation' name='confirmation' type='hidden' value={confirmation} />
											...
											return (
												<>
													<button
														...
														disabled={progress !== undefined || uploading}
													>
														{upload
															? upload.basename
															: progress === undefined
