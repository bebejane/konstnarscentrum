...
export default function Upload(data) {
	...
	const onChange = async (formData) => {
		try {
			...
			for (var pair of formData.entries()) files.push(pair[1]);
			...
			for (let idx = 0; idx < files.length; idx++) {
				...
				res = await axios.put(s3url, file, {
					...
					onUploadProgress: (event) =>
						setProgress((progress) => {
							return {
								...
								totalProgress:
									Math.round((idx / files.length) * 100) +
									Math.round((event.loaded * 100) / event.total) / files.length,
								item: idx + 1,
								total: files.length,
							};
						}),
