...
export default function Members({ members, memberCategories, regions, region: regionFromProps, pagination }: Props) {
	...
	const handleSearch = useCallback(() => {
		...
		abortController.current?.abort('AbortError');
	...
	}, [queryDebounced, regionId, memberCategoryIds, setResults, regions]);

	useEffect(() => {
		handleSearch();
	}, [queryDebounced, regionId, memberCategoryIds, handleSearch]);
	...
	return (
		<div className={s.container}>
			<h1>
				<RevealText>Hitta konstnärer</RevealText>
				<sup className='amount'>{results ? results.length : pagination.count}</sup>
			</h1>
			<div className={s.search}>
				<form className='mid'>
					<span>Namn: </span>
					<input id='search' name='search' type='text' value={query} onChange={(e) => setQuery(e.target.value)} />
					<span>Plats: </span>
...
export const getStaticProps: GetStaticProps = withGlobalProps(
	{
		queries: [AllMemberCategoriesDocument, AllMembersCitiesDocument],
	},
