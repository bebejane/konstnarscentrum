...
(async ()=>{
  ...
  const apiTokens = await Dato.accessTokens.all();
  ...
  fs.writeFileSync('districts.json', JSON.stringify(districts.map(({id, name}) => ({
    id, 
    name, 
    slug:slugify(name, {lower:true}),
    token: apiTokens.filter(t => slugify(t.name, {lower:true}) === slugify(name, {lower:true}))[0]
  }))))
})()