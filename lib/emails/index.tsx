...
import ContactFormConfirmation from '/lib/emails/ContactFormConfirmation';
...
export const portfolioGuidePdfUrl =
	'https://www.datocms-assets.com/55629/1675722618-kc_manual_portfolio.pdf';
...
export const Email = {
		...
		sendMail({
			...
			component: (
				<ResetPassword
					...
					body={
						<>
							Vi har tagit emot din begäran om att ändra ditt lösenord. Använd länken nedan för att
	...
	memberInvitation: ({ email, name, link }: { email: string; name: string; link: string }) =>
		sendMail({
			...
			component: <MemberInvitation name={name} link={link} />,
		}),
	contactForm: ({
		fromName,
		fromEmail,
		...
		sendMail({
			...
			subject,
			component: (
				<ContactForm fromName={fromName} fromEmail={fromEmail} subject={subject} fields={fields} />
			),
		}),
	contactFormConfirmation: ({
		...
		sendMail({
			to,
			subject,
			component: <ContactFormConfirmation name={name} text={text} />,
		}),
	contactFormNotification: ({ datoUrl, name, to }: { to: string; name: string; datoUrl: string }) =>
		sendMail({
			to,
			subject: `Ny medlemsansökning från ${name}`,
			component: <ContactFormRegionNotification name={name} datoUrl={datoUrl} />,
		}),
};
